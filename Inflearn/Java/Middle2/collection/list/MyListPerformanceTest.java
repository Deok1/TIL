...
public class MyListPerformanceTest {
    ...
    public static void main(String[] args) {
        ...
        addMid(new MyArrayList<>(), size);      // 찾을 때: O(1), 데이터 추가(밀기): O(n)
        MyArrayList<Integer> arrayList = new MyArrayList<>();       //조회용 데이터로 사용
        addLast(arrayList, size);               // 찾을 때: O(1), 데이터 추가(밀기): O(1)

        System.out.println("==MyLinkedList 추가==");
        addFirst(new MyLinkedList<>(), size);
        addMid(new MyLinkedList<>(), size);     // 찾을 때: O(n), 데이터 추가: O(1)
        MyLinkedList<Integer> linkedList = new MyLinkedList<>();    //조회용 데이터로 사용
        addLast(linkedList, size);              // 찾을 때: O(n), 데이터 추가: O(1)
    ...
    private static void addMid(MyList<Integer> list, int size) {
        long startTime = System.currentTimeMillis();
        for (int i = 0; i < size; i++) {
            list.add(i/2, i);   // i/2 : 대략 중간쯤으로 계속 데이터를 넣어보는 것
        }
        long endTime = System.currentTimeMillis();
